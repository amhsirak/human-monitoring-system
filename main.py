...
def run():
	...
	while True:
		...
		# moving 'up' or 'down'
		cv2.line(frame, (0, H // 2), (W, H // 2), (3, 252, 240), 3)
		cv2.putText(frame, 'Prediction Border(Entrance)', (10, H - ((i * 20) + 200)),
			cv2.FONT_HERSHEY_SIMPLEX, 0.5, (3, 252, 240), 2)
		...
		for (objectID, centroid) in objects.items():
			...
			cv2.putText(frame, text, (centroid[0] - 10, centroid[1] - 10),
				cv2.FONT_HERSHEY_SIMPLEX, 0.5, (3, 252, 32), 2)
			cv2.circle(frame, (centroid[0], centroid[1]), 4, (3, 252, 32), -1)
		...
		for (i, (k, v)) in enumerate(info2):
			text = '{}: {}'.format(k, v)
			cv2.putText(frame, text, (265, H - ((i * 20) + 60)), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (3, 252, 32), 2)
		...
		if config.Log:
			...
			with open('Log.csv', 'w', newline='') as myfile:
				...
				wr.writerow(('End Time', 'In', 'Out', 'Total People Inside'))
